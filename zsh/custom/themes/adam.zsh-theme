...
prompt_context() {
  if [[ "$USER" != "$DEFAULT_USER" || -n "$SSH_CLIENT" ]]; then
    prompt_segment $ZSH_THEME_DARK default "%(!.%{%F{yellow}%}.)$USER@%m"
  fi
}
  ...
  if $(hg id >/dev/null 2>&1); then
    if $(hg prompt >/dev/null 2>&1); then
      if [[ $(hg prompt "{status|unknown}") = "?" ]]; then
        ...
        # if any modification
        prompt_segment yellow $ZSH_THEME_DARK
        st='±'
      else
      ...
      if `hg st | grep -q "^\?"`; then
        prompt_segment red $ZSH_THEME_DARK
        st='±'
...
prompt_dir() {
  prompt_segment $ZSH_THEME_DIR_DARK $ZSH_THEME_DIR_LIGHT '%~'
}
...
prompt_virtualenv() {
  ...
  if [[ -n $virtualenv_path && -n $VIRTUAL_ENV_DISABLE_PROMPT ]]; then
    prompt_segment blue $ZSH_THEME_DARK "(`basename $virtualenv_path`)"
  fi
}
...
prompt_status() {
  ...
  [[ -n "$symbols" ]] && prompt_segment $ZSH_THEME_DARK default "$symbols"
}
